...
public partial class History : System.Web.UI.Page
{
    ...
    private void GetDataHistory()
    {
        ...
        SqlCommand cmd = new SqlCommand(@"
            SELECT h.id, h.DOCTOR_ID, h.PATIENT_NAME, h.DOCTOR, h.SPECIALTY, h.STATUS, h.DATE_ADDED, h.TIME, h.DATE 
            FROM HISTORY_TB h
            INNER JOIN (
            SELECT PATIENT_ID, DOCTOR_ID, DOCTOR, SPECIALTY, MAX(DATE_ADDED) AS LatestDate
            FROM HISTORY_TB
            WHERE DOCTOR_ID = @UserId
            GROUP BY PATIENT_ID, DOCTOR_ID, DOCTOR, SPECIALTY
            ) latest ON h.DOCTOR_ID = latest.DOCTOR_ID
            AND h.DOCTOR = latest.DOCTOR
            AND h.SPECIALTY = latest.SPECIALTY
            AND h.DATE_ADDED = latest.LatestDate
            WHERE h.DOCTOR_ID = @UserId
            ORDER BY h.DATE_ADDED DESC", con);
    ...
    protected void linkView_Click(object sender, EventArgs e)
    {
        ...
        {
            con.Open();
            string query = @"SELECT * FROM HISTORY_TB WHERE DOCTOR_ID = @doctor_id AND PATIENT_NAME = @patient_name AND SPECIALTY = @specialty ORDER BY id DESC";

            using (SqlCommand cmd = new SqlCommand(query, con))
            {
                cmd.Parameters.AddWithValue("@doctor_id", Session["UserId"]);
                cmd.Parameters.AddWithValue("@patient_name", doctorName);
